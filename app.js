...
const renderSuccessMessage = (mobileNumber, email) => {
    const successMsg = convertToHtmlDom(`<div id="Success">
        <h2>You Have Sucessfully Booked ${selectedSeats.length} Tickets</h2>
        <h4>Booking Details</h4>
        <div class="select">Seats: ${selectedSeats.join(",")}</div>
        <div>Phone number: ${mobileNumber}</div>
        <div>Email: ${email}</div>
    </div>`);

    bookerElement.appendChild(successMsg);
    // let s=document.querySelector(".select").innerHTML;
    // alert(s);
...
function renderConfirmPurchaseForm() {
    ...
    <div id="confirm-purchase">
        ...
        <fieldset>
        <div>
            <label for="email_movie">Email <span>*</span>: </label>
...
const renderMoviesList = async () => {
    ...
    moviesList.forEach(movie => {
        const movieElement = convertToHtmlDom(`<a class="movie-link mname" href="/${movie.name}">
...
renderMoviesList();
// let select=document.querySelector(".available-seat")
// sellect.addEventListener("click",()=>{
//     let s=0;
//     s=s+1;
//     renderSuccessMessage(mobileNumber,email)
// })
